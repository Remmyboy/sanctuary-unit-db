-- A start position for manually hosted 1v1s too.
--
-- A slot is the army number a player starts on — the numbered circle the map's
-- own preview has drawn on it. Only auto matches were given one, because only
-- the lobby the mod creates can act on it; a manually hosted game left it null,
-- so the match room had nothing to say about who starts where and the preview
-- carried no players.
--
-- Every 1v1 gets one now. Nothing enforces it in a manual game — players pick
-- their own slot in the lobby, and some will ignore what the room asks for —
-- but naming the start is worth more than the guarantee, and a wrong guess
-- costs nothing: the game is played wherever they actually sit down.
--
-- Team modes stay unassigned on purpose. A 2v2 or 3v3 map does not order its
-- armies by team, so handing out 1..6 at random would scatter teammates across
-- the map — worse than saying nothing. That needs per-map knowledge of which
-- starts share a corner. (The slot check constraint is still `in (1, 2)`.)
--
-- Otherwise identical to 0011's pass.
...
begin
  ...
  loop
    ...
    for anchor in select * from queue_entries where mode = p_mode order by joined_at loop
      ...
      -- Shuffled independently of hosting and of the split, so neither the
      -- host nor team 1 gets a fixed spawn. 1v1 only — see this file's head.
      ...
      for i in 0 .. v_needed - 1 loop
        ...
        values (v_match_id, v_ids[i + 1],
                ...
                case when p_mode = '1v1'
                  then (case when i = 0 then v_slot1 else 3 - v_slot1 end) end);
      end loop;
